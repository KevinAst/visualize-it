 ...
 * maintaining two counters: the reflexive counts (htmlSnippet
...
export default function createReflectiveCounter(id) {
  ...
  // verify params
  const check = verify.prefix('createReflectiveCounter() parameter violation: ');
  // ... id
  check(id,           'id is required');
  check(isString(id), `id must be a string, NOT: ${id}`);
  ...
  return {
    ...
    monitor(...monitorDependents) {
      ...
      update( (counter) => ({rflx: counter.rflx+1, 
